...
def test_generate_topic_from_tracked_article_rewrites_self_reliance_article_out_of_relationship_expression_sink(
    ...
    assert any(anchor in payload["title"] for anchor in ("稳住", "求助", "自救", "扶稳", "眼前事"))
    assert all(stale not in payload["title"] for stale in ("主心骨", "自己的光", "日子", "下一步"))
    ...
    assert any(anchor in payload["angle"] for anchor in ("求助", "分担", "判断", "行动", "自救"))
    assert all(stale not in payload["angle"] for stale in ("主心骨", "生活怎样重新有了下一步", "日子怎样一点点变亮"))
...
def test_generate_strategy_package_for_self_reliance_tracked_article_uses_self_reliance_lane() -> None:
    ...
    assert "想求助" in payload["problem_brief"]["writing_goal"]
    assert "恢复判断和行动" in payload["problem_brief"]["writing_goal"]
    assert "求助、分担" in payload["problem_brief"]["writing_goal"]
    assert "恢复判断" in payload["strategy_card"]["emotional_path"]
    assert "求助分担" in payload["strategy_card"]["emotional_path"]
...
def test_build_local_tracked_article_draft_fallback_uses_mode_shaped_outline_for_self_reliance_payload() -> None:
    ...
    assert "照顾自己" in body_markdown
    assert "请别人一起分担" in body_markdown
    assert "求助不丢人，自救也不丢人" in body_markdown
    assert "并不是认输" not in body_markdown
    assert "不等于只能硬撑" not in body_markdown
    assert "并不是一个人把所有难处硬熬过去" not in body_markdown
    ...
    for forbidden in ("磨钝", "睡眠", "胃口", "忍住不哭", "束手无策", "每个人都在", "孤立无援", "靠自己，", "。这不是", "很多时候，先把自己扶稳", "愿你以后", "无人可依", "主心骨", "人心里有了光", "日子也会一点点亮起来"):
        assert forbidden not in body_markdown
...
def test_build_local_tracked_article_topic_fallback_uses_distinct_self_reliance_title() -> None:
    ...
    assert any(anchor in topic["angle"] for anchor in ("求助", "分担", "判断", "行动", "自救"))
    assert all(stale not in topic["angle"] for stale in ("主心骨", "生活怎样重新有了下一步", "日子怎样一点点变亮"))
...
def test_build_local_tracked_article_outline_fallback_emotional_acceptance_does_not_reuse_title_as_hook() -> None:
    ...
    assert "常去的那家店" in outline["hook"] or "慢一点" in outline["hook"] or "分寸" in outline["hook"]
...
def test_build_local_publish_package_fallback_emotional_release_uses_mode_lead_and_abstract() -> None:
    ...
    assert package["publish_lead"] == "路过以前常去的那家店时，你还是会下意识慢一点。那段关系没走到最后，却把眼界、分寸和勇气留在了你身上。"
    assert package["abstract"] == "一段相遇停在这里，并不代表它只剩遗憾。你从里面带走的温暖、判断和被照亮过的那一下，会继续留在后来的生活里。"
    assert "有些关系停在半路，也会留下后来用得上的分寸。" in package["intro_options"]
    combined = "\n".join([package["publish_lead"], package["abstract"], *package["intro_options"]])
    for stale in ("最难放下", "讨一个圆满", "别再拿今天去补昨天", "新的日子才会慢慢朝你走来"):
...
def test_emotional_endings_acceptance_packaging_keeps_body_theme_with_concrete_markers() -> None:
    payload = {
        "topic_title": "有些相遇，来过就很好",
        "topic_angle": "从关系阶段性结束切入，写接受离开不是否认，而是把留下的温暖带回自己身上。",
        "reference_article_body_markdown": (
            "成年人的关系，原本就是一段一段的。接纳离开，才是对这段关系最好的祝福。"
            "允许一切发生，也允许一切结束。感谢相遇，不谈亏欠。"
        ),
        "outline": {
            "hook": "",
            "outline_body": "从关系阶段性结束切入，写接受离开不是否认，而是把留下的温暖带回自己身上。",
        },
        "strategy_card": {"structure_mode": "emotional_engine_direct"},
        "problem_brief": {
            "theme_axis": "关系有阶段，结束也可以留下成长。",
            "core_conflict": "人总想证明那段相遇没有白费。",
        },
    }
    title, body_markdown = workbench._build_local_tracked_article_draft_fallback(payload)
    assets_payload = workbench._build_local_assets_fallback(
    ...
    assert workbench._resolve_local_generic_fallback_mode(
        {"source_type": "tracked_article", "topic_title": title, "body_markdown": body_markdown}
    ) == "emotional_engine_direct"
    combined_packaging = "\n".join(
        [
            str(package["publish_lead"]),
            str(package["abstract"]),
            *[str(option) for option in package["intro_options"]],
        ]
    )
