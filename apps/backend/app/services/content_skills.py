...
WECHAT_PLATFORM_VALUE_SKILL = ContentSkill(
    ...
    instructions_by_stage={
        "topic": (
            ...
            "不要把生活场景作为默认入口，优先用终局问题、反常识判断、情绪命名或价值赦免打开。"
            "不要生产疑似投机的低创作度内容。"
        ),
        "outline": (
            "平台鼓励具有丰富信息含量、信息增量或情绪价值的内容。"
            "大纲必须规划清楚每一段给读者新增什么：新观察、具体处境、原因机制、情绪承接、行动落点或现实提醒。"
            "原则上不规划场景段，先规划情绪发动机：终局感、亏欠感、失去后的反省、被允许的松绑或现实答案。"
            "需要例证时只保留一句事实或结果，不展开环境、动作、物件和氛围描写。"
        ...
        "draft": (
            ...
            "正文默认去掉场景描写，不用生活场景冷启动，不连续写动作、环境、物件或氛围。"
            "需要例证时只保留一句事实或结果，并立刻接上情绪命名、矛盾拆解、现实机制或行动答案。"
            "例证不能单独成段，不写“手机亮一下”“电梯门开了”这类独立动作残留。"
...
DIRECT_ANSWER_SCENE_BUDGET_SKILL = ContentSkill(
    ...
    instructions_by_stage={
        "topic": (
            "表达要直接，不要含蓄绕弯。"
            "标题和切入角度先给问题、判断、终局感或答案入口，不要用场景氛围包装选题。"
        ),
        "outline": (
            "大纲默认不规划场景描述。"
            "单段优先写问题、判断、原因、行动或情绪承接。"
            "如需例证，只写一句事实或结果，不展开生活表面陈列。"
            "结尾要明确结论、边界或行动落点，不要含蓄留白。"
        ),
        "draft": (
            "正文不需要含蓄，默认去掉场景描写。"
            "不要用生活场景冷启动，不要连续铺动作、环境、物件和氛围。"
            "例证最多一句，写完立刻回到判断、机制、答案或行动落点。"
            "例证不能单独成段，不能保留孤立动作残留。"
