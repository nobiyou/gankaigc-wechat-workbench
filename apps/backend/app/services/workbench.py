...
def _build_local_supportive_appreciation_paragraphs(
    *,
    ...
    if _has_local_supportive_misread_profile(payload):
        ...
        return [
            opening,
            "一句“对不起”落下来，她愿意把这件事往后放一放，不代表她真的什么都不介意。很多时候，她只是把情分看得比一时的输赢更重。",
...
def _resolve_local_fallback_mode(payload: Mapping[str, object]) -> str:
    ...
    if any(
        token in f"{title} {corpus}"
        for token in ("没时间", "时间在哪儿", "时间分配", "排在前面", "评论", "点赞", "追问", "补问", "言外之意", "读懂", "轻互动", "回消息")
    ):
        return "response_priority"
    if any(
        token in f"{title} {corpus}"
...
_LOCAL_MODE_OUTLINE_DEFAULTS: dict[str, tuple[str, str, str, str]] = {
    ...
    "emotional_engine_direct": (
        ...
        "等你肯把感谢留下、把那段路安放回过去，新的日子才会慢慢朝你走来",
    ),
...
def _build_local_self_worth_rebuild_paragraphs(
    *,
    ...
    return [
        intro,
        "很多人不是没有底线，只是太习惯先把场面顾过去。别人临时改主意，你先说行；不合适的请求递过来，你也总想再扛一下。",
        "时间久了，别人会以为你好商量，你自己也会差点忘了，那些不想答应、不想将就的感觉，本来就该算数。",
        "所谓把自己养贵一点，不是突然端着，也不是把门一关谁都不理。是开始知道，什么关系值得花时间，什么要求不必硬着头皮接。",
        "门槛不是拿来为难别人的，是提醒自己：别再为了显得懂事，把尊重和体面一并让掉。",
        "你把标准慢慢收回来，不会失去真正珍惜你的人。恰恰相反，他会因为你终于认真对待自己，知道该怎样认真对待你。",
        "关系里最怕的，从来不是你有要求，而是你连自己都默认：随便一点也行。",
        "把自己放回前面，不是高傲，是清醒。你先把自己看重，别人才能慢慢学会看重你。",
    ]
...
def _build_local_response_priority_time_priority_paragraphs(
    *,
    ...
    return [
        ...
        "一个人把时间给谁，往往不是嘴上说出来的，是那些细小空当里先想到谁。愿意把回应补回来的人，心里一直有你的位置。",
        "真正的在意，从来不是二十四小时黏着你。是明明有很多事，也还是会记得别让你一直等，别让你总在那边替他找理由。",
        "把真心留给愿意回应你的人。不是催谁，而是别再让自己一直在等待里，把位置慢慢放轻。",
...
def _build_local_mode_shaped_generic_paragraphs(
    *,
    ...
    if mode == "everyday_warmth_return":
        return [
            intro,
            "鞋还没换好，厨房里有人探头说：“回来啦？先洗手。”这句话不响，却像把一天的风尘轻轻拍了拍。",
            "年轻时总觉得幸福要有很大的样子：账户数字再漂亮一点，房子再大一点，朋友圈再热闹一点。",
            "可人走到后来，会被很小的事劝住。",
            "父母电话里一句“别太累”，朋友饭桌上一句“你先说完”，孩子回头喊你一声，心就落了地。",
            "原来真正让人羡慕的一幕，往往就是有人还肯这样惦记你。",
            "有个朋友前阵子说，他最开心的一天，没有升职，也没有买什么贵东西。",
            "只是下班早了半小时，陪父母去菜市场买了一把青菜。回家时，孩子在楼下等他，手里攥着一根快化的冰棍，非要分他一口。",
            "饭桌上没什么大菜，父亲问他最近睡得好不好，母亲把鱼肚子那块夹到他碗里。",
            "那一刻，他忽然觉得，自己这几年追得那么急，其实想要的不过是这样的晚上。",
            "朋友不用很多。能在你话说到一半时不急着评价，能在你沉默时问一句“是不是累了”，就已经很难得。",
            "家也不一定要多大。推门有人应，饭桌有你的位置，生病时有人递水，天冷时有人催你加衣，这就是很多人一生想守住的福气。",
            "苏轼写过一句：“人间有味是清欢。”",
            "这句话好，是因为它把幸福从高处请回了日常。",
            "家里少点挂心事，三两老友还在，饭能趁热吃，话能慢慢说。",
            "这些小事放在年轻时不觉得贵，走过一些风雨才知道，它们样样都难得。",
            "年轻时看这些，总觉得太普通。等到真正忙过、病过、和一些人走散过，才知道普通两个字很难得。",
            "能吃一顿安稳饭，能和老友说几句闲话，能听见家里有人笑，心里那口气就会松下来。",
            "人生最好的状态，是忙完一天，还有心情把一碗汤喝热。",
            "今晚就别急着和世界比输赢了。",
            "把饭吃热，把话说慢。家里人平安，知己还在，日子已经很值得。",
        ]
    if mode == "inner_settlement":
    ...
    if mode == "emotional_engine_direct":
        ...
        return [
            intro,
            "更磨人的，往往是日子已经往前走了，你还是会在某个寻常时刻停一下。路过熟悉的地方，听见一句像他的话，或者只是傍晚风一吹过来，心里那块地方还是会轻轻发酸。",
            "你反复想起的，也未必是一定要回去。更多时候，只是舍不得把一段认真过的相遇，轻易算成白忙一场。",
            "可人和人走散，并不会把那段路上得到的东西一并带走。你后来更懂分寸了，更知道自己要什么了，也更明白被好好对待是什么感觉。",
            "这些留下来的部分，没有消失。它们安静长在你身上，变成你现在看人的眼光，做选择的底气，还有重新生活的能力。",
            "所以，偶尔想起也没关系。那点舍不得会慢慢退下去，等你不再急着向结局讨一个说法，心反而会一点点松开。",
            "把感谢收好，把关系放回过去，把今天认真过完。那段路安放好了，新的日子才会慢慢朝你走来。",
        ]
    if mode == "supportive_appreciation":
    ...
    if mode == "relationship_aftercare":
        return [
            intro,
            "门关上以后，桌上的水杯还在原处。",
            "两个人都没再吵，可屋里比刚才更冷。刚才那些重话像没收干净的碎片，谁走过去都会被扎一下。",
            ...
            "好的关系当然会有争执。可更重要的是，争执以后谁还愿意把门重新打开。",
            "愿意回来的人，会先把声音放低，会承认刚才哪句话说重了，也会问一句：你刚刚是不是很难受。",
            "这不是输赢，是把关系从那阵冷气里拉回来。",
            "心里有这段关系的人，不会让你独自站在那阵冷气里。",
    ...
    if mode == "pressure_interface_direct":
        return [
            ...
            "日子表面还照常走，顺序却一点点乱了。该吃饭的时候先回消息，该休息的时候再看一眼表格，该复查的时候又给自己找了个理由。",
...
def _resolve_local_assets_social_teaser(
    *,
    ...
    tail_map = {
        ...
        "self_reliance_inward_support": "先把眼前这一步走稳，明天的事再一件件处理。",
        "self_worth_rebuild": "别让那句“都可以”，替你让掉自己的位置。",
            ...
            if _uses_local_self_reliance_shared_burden_variant(focus_payload):
                publish_lead, abstract = _pick_local_seeded_pair_variant(
                    ...
                    (
                        ...
                        (
                            "人最需要支撑的时候，也可以先把眼前这一小步走稳。主心骨回来以后，很多话才知道该怎么说，很多事也有了下一步。",
                            "真正的自我支撑，不是把门关上一个人扛完，而是先让自己亮起来。你能往前走，也能在合适的时候请人一起分担。",
                        ),
                        (
                            "有些时刻，先把一件小事做稳，心里的光就会回来一点。等那点光亮起来，人就不再只剩慌张。",
                            "把力气收回自己手里，是为了更清楚地生活，也更坦然地求助。人有了主心骨，风再大，也能慢慢站稳。",
                        ),
                    ),
                )
            else:
                publish_lead, abstract = _pick_local_seeded_pair_variant(
                    ...
                    (
                        ...
                        (
                            "有些日子不需要立刻想通全部答案。先把今天这一点光守住，人就能重新稳住脚步。",
                            "日子真正往前的时候，往往不是忽然变轻，而是你重新看见了下一步。看见下一步，心里就有了光。",
                        ),
                    ),
                )
        ...
        if mode == "everyday_warmth_return":
            ...
            self_reliance_second_intro = _pick_local_seeded_text_variant(
                ...
                (
                    ...
                    "能重新看见下一步，人就不只是被难处推着走。",
                    "等心里有了方向，再开口、再分担，都会更从容。",
                ),
            )
            intro_options = _dedupe_nonempty_text_options(
                [
