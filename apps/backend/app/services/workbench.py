    ...
    *,
    ...
    if mode == "everyday_warmth_return":
        if payload and _uses_local_everyday_warmth_simple_happiness_variant(payload):
            ...
            return [
                ...
                "知己也不用成群。能在你把话说到一半时不抢着下结论，能在你沉默时陪你坐一会儿，能在你高兴时真心替你高兴，三两个就够珍贵。",
    ...
    if mode == "scene_first_progression":
        ...
        if has_meeting_room:
            return [
                ...
                "等人散得差不多了，你再去想刚才那几句话该怎么说，已经不像表达，更像一个人给自己补课。你反复回放那个瞬间，问题还在那里，心里更堵的是自己又把场面放在了前面。",
                "话在会上咽回去一次，下一次就更容易继续往后放。判断你一直有，问题你也看见了。只是每回轮到自己开口，你总先把场面稳住，再把自己那句更重要的留到最后。",
                "这样久了，能力还在，先退掉的却是那股“我也可以在这里说话”的劲。你太习惯先顾全气氛，慢慢就连自己都忘了，那句话本来就该在当场出现。",
                "话留在当场，人才站得稳。",
                "很多位置，要靠一次次当场开口慢慢站出来。你肯把那句话留在当场，别人才能慢慢听见你，你自己也会更站得住。下次再进那间会议室，不用一下子说很多。把最该说的那一句留住，就已经是在往前走了。",
            ]
    ...
    if mode == "emotional_engine_direct":
        ...
        if payload and _uses_local_emotional_forgiveness_release_variant(payload):
            return [
                intro,
                "那件不痛快的事隔了很久，还是会在夜里冒出来。明明灯已经关了，脑子却又把当时那句话翻出来，越想越堵。",
                "对方早就过完了今天，你却还在旧事里来回走。一个本该睡踏实的夜晚，就这样又交给了旧怨。",
                "那件事当然需要被看清，委屈也不必被抹掉。可从某一天起，你可以把它放到该放的位置：记住教训，收回分寸，然后继续过自己的日子。",
                "放过别人，说到底是把自己从旧事里领出来。把无足轻重的争执、误会和旧伤慢慢清出去，今天才有地方晒太阳，也有地方重新种花。",
...
def _resolve_local_assets_social_teaser(
    *,
    ...
    if mode == "emotional_engine_direct" and _uses_local_emotional_forgiveness_release_variant(payload):
        lead = first if first_is_safe else "那件不痛快的事隔了很久，还是会在夜里冒出来。"
        return _compose_local_followup(lead, "把旧怨放回旧处，眼前的日子才会重新照进光。")
    if mode == "emotional_engine_direct" and _uses_local_emotional_memory_reflux_variant(payload):
    ...
    if responsibility_focus:
        ...
        if mode == "response_priority":
            ...
            if _uses_local_emotional_regret_forward_variant(focus_payload):
                ...
                publish_lead = "那件不痛快的事隔了很久，还是会在夜里冒出来。把旧怨慢慢放下，眼前的日子才有地方重新照进光。"
