...
DEFAULT_TONE_PROFILE_PRESET = BuiltinToneProfilePreset(
    ...
    paragraph_rhythm="中短段直接推进；不靠整段场景铺陈，但允许少量现实细节穿针引线，每段都要带来新判断、机制、情绪承接或行动落点",
...
JINWAN_YOUYU_PRESET = BuiltinToneProfilePreset(
    ...
    paragraph_rhythm="标准三段式直接推进，中段围绕 2 到 4 个明确判断展开；每个判断都要给出依据、情绪承接或行动落点，不靠整段氛围铺陈，但允许少量现实细节带路",
...
JINWAN_YOUYU_INTERNAL_PRESSURE_PARAGRAPH_RHYTHM = (
    "先用真实接口带路，再给判断与落点；中段可以明确，但不要一上来就写成通用讲解稿，"
