...
JINWAN_YOUYU_PRESET = BuiltinToneProfilePreset(
    ...
    paragraph_rhythm="标准三段式直接推进，中段围绕 2 到 4 个明确判断展开；多数段落以 1 到 3 句为主，遇到动作变化、转折或落点就拆段；每个判断都要给出依据、情绪承接或行动落点，不靠整段氛围铺陈，也不要把几层意思压进一个长段，但允许少量现实细节带路",
    ...
    default_polish_instruction=(
        ...
        "并补出新的判断依据、情绪承接或行动落点，"
        "多数段落控制在 1 到 3 句，单段如果同时塞了现象、解释、转折和落点，就主动拆开，"
...
JINWAN_YOUYU_INTERNAL_PRESSURE_VALUE_CONSTRAINTS = (
    "如果题材是自我消耗、生活排序失衡、健康透支或身体提醒，"
    "必须先给出一个具体接口、后果或身体信号，再给判断；"
    "不要把话题抽成空泛答案句，必须让读者先认出自己。"
)

JINWAN_YOUYU_INTERNAL_PRESSURE_POLISH = (
    "如果稿子是自我消耗、生活排序失衡、健康透支或身体提醒题材，"
    "请保留直接感，但把空泛答案句改成真实接口、后果或身体信号；"
    "不要让开头和结尾都先端出结论，先让读者看见具体代价，再给行动落点。"
    "多数段落控制在 1 到 3 句，段落职责一变就拆，不要为了显得完整把几层意思压成一个长段。"
)
