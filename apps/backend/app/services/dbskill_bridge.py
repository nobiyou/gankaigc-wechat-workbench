...
_LOCALIZED_RULE_REPLACEMENTS = {
    "如果一句话还说不清楚，先退回到具体场景、动作和顺序，不要急着下结论。": (
        "如果一句话还说不清楚，先退回到现实接口、读者处境和代价承接，"
        "不要急着铺场景或下结论。"
    ),
    "不要只给概念命名，要让每个判断都落到具体用法、动作或关系变化上。": (
        "不要只给概念命名，要让每个判断都落到压力推进、关系变化或现实后果上。"
    ),
    "每一节都要能落到具体场景、动作或关系变化，不能只摆概念。": (
        "每一节都要能落到压力推进、关系变化或现实后果，不能只摆概念，也不能用场景描写凑篇幅。"
    ),
    "遇到匀速排比和整齐翻转时，优先把句子拉回动作、停顿和关系变化。": (
        "遇到匀速排比和整齐翻转时，优先把句子拉回现实接口、判断推进和关系后果。"
    ),
    "开头先给一个抓手：动作、界面、物件、空间距离或身体反应，先别下总判断。": (
        "开头先给一个现实抓手：接口、后果、身体提醒或当下卡点，"
        "不用生活场景冷启动。"
    ),
...
_DEFAULT_TRACKED_ARTICLE_RULES: dict[str, Any] = {
    ...
    "strategy": {
        ...
        "execution_checklist": [
            "问题说明书里是否已经把大词拆成现实接口、读者处境或代价推进，而不是继续停在抽象概念上。",
    ...
    "draft": {
        ...
        "execution_protocol": [
            "开头先给一个现实抓手：接口、后果、身体提醒或当下卡点，不用生活场景冷启动。",
            "不要写平台化的“钩子 + 痛点 + 承诺”三件套开头，直接进入要解释的处境。",
            "不要段段收束、段段出金句，至少留一段只推进观察、接口变化或关系后果。",
            "每次只保留一个最想强调的判断，其余判断埋回过程、压力推进和现实后果里。",
            "结尾回到一个明确结论、关系余波或现实阻力，不要祝福式收尾，也不要另补小动作。",
        ],
        "self_checklist": [
